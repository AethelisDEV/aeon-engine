    ...
    pub fn draw_animation_section(
        ...
        if let Ok(player) = world.get::<&ae_animation::AnimationPlayer>(entity) {
            ui.group(|ui| {
                ...
                ui.horizontal(|ui| {
                    ui.label(
                        egui::RichText::new("🎬 Animation Player")
                            .strong()
                            .color(egui::Color32::WHITE),
                    );
                    ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                        if ui
                            .button("🗑")
                            .on_hover_text("Remove AnimationPlayer")
                            .clicked()
                        {
                ...
                ui.horizontal(|ui| {
                    ui.label("Timeline:");
                    let progress_pct = (updated.current_time / duration * 100.0).clamp(0.0, 100.0);
                    if ui
                        .add(
                ...
                let model_anims = if let Ok(model_id) = world.get::<&ae_core::ecs::ModelId>(entity)
                {
                ...
                if let Ok(skel) = world.get::<&ae_animation::Skeleton>(entity) {
                    ...
                    ui.colored_label(
                        egui::Color32::GOLD,
                        "ℹ Static 3D Model (No Skeletal Armature/Bones found).\nExport with Blender Armature/Bones to use skeletal animation.",
                    );
                }
